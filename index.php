...
// Responder rápido a preflight CORS (el navegador manda OPTIONS antes del POST real)
...
// Certificado digital: En producción usa tu .pem real
// Para pruebas, usamos el certificado que viene incluido gratis en Greenter
$certPath = getenv('CERT_PATH') ?: __DIR__ . '/data/certificate.pem';
if (!file_exists($certPath)) {
    // Fallback al certificado de prueba incluido en Greenter
    $certPath = __DIR__ . '/vendor/greenter/xml/src/Resources/cert.pem';
    if (!file_exists($certPath)) {
        // Buscar en otra ruta posible
...
$invoice->setDetails($details)
    ->setLegends([
        (new Legend())->setCode('1000')->setValue(strtoupper($leyenda))
    ]);
