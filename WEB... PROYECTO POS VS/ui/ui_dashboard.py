...
class DashboardUIManager:
    ...
    def build_ui(self) -> None:
        ...
        self.lbl_clock = tb.Label(
            clock_frame,
            text="00:00:00",
            font=("Consolas", 24, "bold"),  # type: ignore[arg-type]
            bootstyle="inverse-dark",
        )
        self.lbl_clock.grid(row=0, column=0, sticky="ew", pady=(8, 0))
        self.lbl_date = tb.Label(
            clock_frame,
            text="--",
            font=("Segoe UI", 10),
            bootstyle="inverse-dark",
        ...
        self.lbl_motivation = tb.Label(
            clock_frame,
            text="Listo para vender",
    ...
    def _create_rus_card(self, parent: tk.Widget, column: int) -> None:
        ...
        self.progress_rus = tb.Floodgauge(
            ...
            bootstyle="success",
            mask="S/ {}",
            maximum=self._rus_limit,
            font=("Segoe UI", 12, "bold"),  # type: ignore[arg-type]
        )
    ...
    def _handle_manual_refresh(self) -> None:
        btn = self.refresh_button
        if btn and btn.winfo_exists():
            btn.configure(state="disabled")
        self.refresh_dashboard()
        if btn and btn.winfo_exists():
            btn.after(800, lambda b=btn: b.configure(state="normal"))
        ...
        try:
            ...
            if callable(get_summary_for_day):
                ...
                if isinstance(summary, tuple) and len(summary) == 2:
                    yesterday_count, yesterday_total = summary
                else:
    ...
    def _update_rus_gauge(self, month_total: float) -> None:
        if not self.progress_rus:
            return
        value = max(0.0, min(month_total, float(self._rus_limit)))
        gauge_value = int(round(value))
        self.progress_rus.configure(value=gauge_value)
    ...
    def update_clock(self) -> None:
        try:
            ...
            if self.lbl_motivation and now.second % 12 == 0:
                self.lbl_motivation.configure(text=random.choice(self._motivation_messages))

            cycle = self._palette.get("clock_cycle", [])
            if cycle and self.lbl_clock:
                self.lbl_clock.configure(foreground=cycle[now.second % len(cycle)])
